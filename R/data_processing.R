...
data <- read_csv(here('output','data_pull.csv')) %>% 
  mutate(
...

{
  ...
  ggsave(
    p2,
    filename = here('output', 'pfas_by_state_water_tissue_p2.png'),
    width = 20,
    height = 12.5,
    units = 'in',
    dpi = 500,
...
# Data Processing --------------------------------------------------------
{
# ! NOTE: SXT: Not touching this section, just reviewing it.
...
# ! NOTE: SXT: This function is computationally intensive and may take a while to run.
# ! Seems to hang? Wasn't able to run it on my machine.
...
data_12 <- TADA_SimpleCensoredMethods(
  data_11a,
...
data_14 <- data_13 %>%
  mutate(
    negative_value_flag = ifelse(TADA.ResultMeasureValue >= 0, 'PASS', 'FAIL')
  )
...
data_15 <- data_14 %>%
  mutate(
    result_dection_name_flag = ifelse(
...
summary(as.factor(
  data_15$TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode
))


data_16 <- data_15 %>%
  mutate(
    detection_limit_numeric = case_when(
      TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode == 'UG/L' ~
    ...
    detection_limit_units = case_when(
      TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode == 'UG/L' ~ 'NG/L',
      TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode == 'NG/ML' ~
        'NG/L',
...
data_17 <- data_16 %>%
  ...
  mutate(
    detection_limit_value_flag = case_when(
      detection_limit_numeric >= 2 * DL_std + DL_avg &
        (is.na(TADA.ResultMeasureValue) == T |
          TADA.CensoredData.Flag == 'Non-Detect') ~
        'FAIL',
    ...
    sample_lower_than_detection_limit_flag = case_when(
...
data_18 <- data_17 %>%
  mutate(
    EPA_method_flag = case_when(
      ResultAnalyticalMethod.MethodDescriptionText == 'EPA Method 537.1' &
      ...
      ResultAnalyticalMethod.MethodDescriptionText == 'EPA 533' &
        ...
        'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '533' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '1633' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '537.1' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '8327' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '537.1M-MDEQ-WQ' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == 'EPA 537.1' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '537 Modified' ~ 'PASS',
      ResultAnalyticalMethod.MethodIdentifier == '537.1M-MDEQ-WQ' ~ 'PASS',
      T ~ 'FAIL'
    )
  )
...
write_csv(data_18, 'output/EPATADA_Original_data_with_flags_tags.csv')
 ...
 # ! NOTE: Read in using saved files
data_18 <- read_csv(here('output', 'EPATADA_Original_data_with_flags_tags.csv')) 

#Find non-detect detection limit range
summary(subset(
  data_18$TADA.DetectionQuantitationLimitMeasure.MeasureValue,
...
samples_filtered <- data_18 %>%
  filter(
    TADA.ResultUnit.Flag != "Rejected" & TADA.ResultUnit.Flag != "Invalid"
  ) %>% # Step 1; critical. 0 samples.
  filter(
    TADA.SampleFraction.Flag != "Rejected" &
      TADA.SampleFraction.Flag != "Invalid"
  ) %>% # Step 2; critical. 0 samples.
  filter(
    TADA.MethodSpeciation.Flag != "Rejected" &
      TADA.MethodSpeciation.Flag != "Invalid"
  ) %>% # Step 3; critical. 0 samples.
  filter(
    TADA.AnalyticalMethod.Flag != "Rejected" &
      TADA.AnalyticalMethod.Flag != "Invalid"
  ) %>% # Step 7
  filter(
    TADA.SingleOrgDupGroupID == "Not a duplicate" |
      (TADA.SingleOrgDupGroupID != "Not a duplicate" &
        TADA.SingleOrgDup.Flag == "Unique")
  ) %>% # Step 8; 57712 to 52942. SPOT CHECK
  filter(TADA.ActivityType.Flag == 'Non_QC') %>% # Step 9; critical. 52942 to 50102.
  filter(
    TADA.MeasureQualifierCode.Flag != 'Suspect' |
      is.na(TADA.MeasureQualifierCode.Flag)
  )
...
states_w_data <- samples_filtered %>%
  left_join(., state_num, by = c('StateCode' = 'state_code')) %>% 
  group_by(state_name) %>%
  mutate(n_samples_total = n()) %>%
  ungroup() %>%
  ...
  unique() %>% 
  left_join(., state_num, by = c('state_name' = 'state_name'))

{
  ...
  ggsave(
    p3,
    filename = here('output', 'pfas_by_state_water_tissue_p3.png'),
    width = 20,
    height = 12.5,
    units = 'in',
    dpi = 500,
